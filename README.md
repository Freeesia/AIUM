...
Codex 側の認証・利用状況取得には、Codex アプリの login client ID と ChatGPT backend の Codex usage endpoint を利用します。
...
利用状況取得は `https://chatgpt.com/backend-api/wham/usage` を呼び出し、保存済みの `accountId` がある場合は `ChatGPT-Account-Id` ヘッダーに付与します。認証トークンから取得できた `accountId` / `email` / `name` は Keychain 内の Codex token bundle に保存します。設定画面と使用量カードではアカウント名を優先し、名前を取得できない場合はメールアドレスを表示します。既存のログイン情報も保存済みトークンから名前を読み取るため、トークンに名前が含まれていれば再ログインは不要です。
...
アプリのバージョンは [GitVersion](https://gitversion.net/) を使って Git 履歴・タグから自動生成します。
...
App Store へ公開するバージョンをリリースする場合は、`main` ブランチに `vX.Y.Z` 形式のタグを打ちます。
...
タグを打つと TestFlight ワークフローが実行され、対応するバージョン（例: `1.2.0`）で公開用の TestFlight ビルドとしてアップロードされます。

### タグなし通常ビルドのバージョン

最後のタグ以降のコミットは GitVersion が `Patch` を自動インクリメントします。たとえば最後のタグが `v1.2.0` で 3 コミット進んでいる場合、`MARKETING_VERSION` は `1.2.3` になります。

```sh
# 現在のバージョンを確認（GitVersion がインストール済みの場合）
dotnet gitversion
```
