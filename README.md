...
- GitHub 側は OAuth Device Flow と GitHub Billing API を利用しますが、利用状況 API はアカウント、契約プラン、課金主体によって取得できない場合があります。
...
Organization で管理・課金されている Copilot seat の使用量を取得する場合は、設定画面の `Billing Organization` に organization slug を入力し、Organization permissions の `Administration` を `Read-only` にした token を保存します。保存した token は Keychain に保存され、GitHub 使用量取得に使われます。
